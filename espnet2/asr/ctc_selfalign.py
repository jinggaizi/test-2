...
class CTC(torch.nn.Module):
    ...
    def loss_fn(self, th_pred, th_target, th_ilen, th_olen) -> torch.Tensor:
        if self.ctc_type == "builtin":
            ...
            import pdb
            #pdb.set_trace()
            for idx, olen in enumerate(th_olen):
                first_cor_token = th_target[sum_idx]
                length = th_pred[:,idx,:].size(0)
                index_ori = np.arange(length)
                for t in range(length):
                    corr_topk1 = th_pred[:,idx,:][t].topk(1)[1]
                    if first_cor_token == corr_topk1:
                        fix_time = t
                        #pdb.set_trace()
                        break
                #if t == length -1:
                ...
                #pdb.set_trace()
                try:
                    th_pred_fix.append(th_pred[:,idx,:].index_select(0, torch.tensor(index_ori, dtype=torch.long, device=th_pred.device)))
                except:
                    #pdb.set_trace()
                    pass
                #print(len(th_pred_fix))
                #pdb.set_trace()
                sum_idx += olen
            #pdb.set_trace()
            if len(th_pred_fix) == th_pred.size(1):
                th_pred_left = torch.stack(th_pred_fix, 1)
                #assert th_pred.size(1) == th_pred_left.size(1)
            #pdb.set_trace()
            th_target = th_target.cpu().int()
            th_ilen = th_ilen.cpu().int()
            th_olen = th_olen.cpu().int()
            loss = self.ctc_loss(th_pred_left, th_target, th_ilen, th_olen)
            #loss = self.ctc_loss(th_pred, th_target, th_ilen, th_olen)
            if self.reduce:
