...
def subsequent_chunk_mask(
    ...
    Args:
        size (int): size of mask
        chunk_size (int): size of chunk
        left_frames (int): number of left frames
            <0: use full chunk
            >=0: use left_frames
        device (torch.device): "cpu" or "cuda" or torch.Tensor.device
    ...
    for i in range(size):
        if left_frames < 0:
            ...
            start = max(i // chunk_size * chunk_size - left_frames, 0)
        ending = min((i // chunk_size + 1) * chunk_size, size)
        ret[i, start:ending] = True
    return ret
...
def add_optional_chunk_mask(xs: torch.Tensor, masks: torch.Tensor,
    ...
    if use_dynamic_chunk:
        ...
        if decoding_chunk_size < 0:
            ...
            chunk_size = decoding_chunk_size
            left_frames = decoding_left_frames
        else:
            # chunk size is either [1, 25] or full context(max_len).
            # Since we use 4 times subsampling and allow up to 1s(100 frames)
            # delay, the maximum frame is 100 / 4 = 25.
            ...
            if chunk_size > max_len // 2:
                ...
                if use_dynamic_left_chunk:
                    ...
                    if max_left_chunks == 0:
                        ...
                        left_frames = chunk_size * torch.randint(0, max_left_chunks,
                                                        (1, )).item()
        chunk_masks = subsequent_chunk_mask(xs.size(1), chunk_size,
                                            left_frames,
                                            xs.device)  # (L, L)
